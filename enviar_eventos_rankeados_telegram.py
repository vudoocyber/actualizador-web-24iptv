...
def es_fin_de_semana():
    ...
    Lunes=0, Domingo=6.
    """
    # Importante: datetime.now() usa la variable de entorno TZ: America/Mexico_City
    hoy = datetime.now(MEXICO_TZ).weekday() 
    # Sábado es 5, Domingo es 6 (según la definición de Python donde Lunes es 0)
    return hoy >= 5 
...
def formatear_mensaje_telegram(evento):
    ...
    # Filtramos plantillas: si es entre semana, solo usamos plantillas donde SPECIAL_FIN_SEMANA es False.
    if es_weekend:
    ...
    # Si el filtro devuelve una lista vacía (ej. solo hay plantillas de fin de semana), usamos la genérica.
