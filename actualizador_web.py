...
⭐ 24IPTV & HomeTV – Tu Mejor Elección en Entretenimiento Deportivo ⭐"""
    ...
    # Retornamos AMBOS: HTML para la web y Texto Puro para TXT de Telegram
...
# --- 5. FUNCIÓN PARA COMUNICARSE CON GEMINI (RESTAURADA) ---
def obtener_ranking_eventos(texto_crudo):
    # Ya que el ranking se ha delegado, esta función no hace nada aquí.
    if 'GEMINI_API_KEY' in globals() and globals()['GEMINI_API_KEY']:
        ...
        print("ADVERTENCIA: La API Key de Gemini no está disponible.")
    return []

# --- 6. FUNCIÓN PARA CREAR JSON DE EVENTOS ---
def crear_json_eventos(texto_crudo, ranking):
    # Esta es la lógica original que asumo que tu otro script consume.
    data = {
        "fecha_extraccion": datetime.now().isoformat(),
        "contenido_texto_crudo": texto_crudo,
        "ranking_eventos": ranking
    }
...
def main():
    ...
    # La función aplicar_reglas_html y su resultado han sido eliminados de aquí
    ...
    try:
        ...
        # NOMBRE_ARCHIVO_PROGRAMACION (programacion.html) ya no se guarda
